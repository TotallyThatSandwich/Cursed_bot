...
class genericAI(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if "reply" in str(message.type):
            ...
            if (str(repliedMessage.author.id) == "1210389365185056818" or str(repliedMessage.author.id) == "1210512184103403530") and not(message.author.id == "1210389365185056818" or message.author.id == "1210512184103403530"):
                response = await formatResponse(botQuery)
                return await message.channel.send(response)
